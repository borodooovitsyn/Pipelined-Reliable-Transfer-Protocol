...
python3 prtp_receiver.py \
  --bind-ip 127.0.0.1 --bind-port 9206 \
  --out "$OUT" --loss-ack 0 --corrupt-ack 0.10 &
RPID=$!

sleep 0.2

python3 prtp_sender.py \
  --server-ip 127.0.0.1 --server-port 9206 \
  --file test.bin --loss 0 --corrupt 0.10
